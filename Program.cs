...
namespace YoutuBot
{
    ...
    {
        private static async Task<string> MakeNarRequest(string otp, string formKey, string sessionId)
        {
            ...
            using (var httpClient = new HttpClient(handler))
            {
                using (var request = new HttpRequestMessage(new HttpMethod("POST"),
                    "https://plus.nar.az/user/account/validateotp"))
                {
        ...
        private static async Task Main(string[] args)
        {
            ...
            var i = 1;
            foreach (var videoInfo in videos) Console.WriteLine(i++ + ") " + videoInfo.Title);
        }
    }
}